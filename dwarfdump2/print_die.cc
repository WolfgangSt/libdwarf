    ...
    SrcfilesHolder & hsrcfiles)
{
    ...
    for (;;) {
        ...
        bool retry_print_on_match = 
            print_one_die(hin_die, info_flag,indent_level, hsrcfiles,
            /* ignore_die_printed_flag= */ false);
                                    ...
                                    Dwarf_Error * err)
{
    ...
    if (vres != DW_DLV_OK) {
        ...
        if (vres != DW_DLV_OK) {
            if (string_out != 0) {
                string v = attr_name + string(" has a bad form.");
                *string_out = v;
            }
            return vres;
        }
...
get_FLAG_BLOCK_string(Dwarf_Debug dbg, Dwarf_Attribute attrib)
{
    ...
    while (array_remain > array_lim) {
        ...
        for(unsigned j = 0; j < array_lim; ++j) {
          out_str.append(blank + IToHex(array_ptr[0],8));
        }
        array_ptr += array_lim;
    ...
    if (array_remain > 0) {
        ...
        while (array_remain > 0) {
            out_str.append(blank + IToHex(*array_ptr,8));
            array_remain--;
            array_ptr++;
        }
    }
